...
```
Users write the amount of the RWA they want to exit and choose the stablecoin they're exiting to. They then click "Swap" on the UI, which needs to trigger a "CREATE" request above. Solvers will then bid on it offchain and return bids to you without you having to do anything. You can get them via the GET endpoint above and show whichever one(s) you want to the user. We recommend you show the user the info about the best bid available - settlement time, price, duration covered and total slippage. The user then accepts the bid displayed, which triggers swap settlement - a transaction that will swap their assets with the winning solver's assets. Build a UI enabling this user flow and you will have created an instant redemption experience! See demo.octarine.finance for an example of such an experience.

In practice, a **request** is the user's intent, keyed by `requestId` (a UUID). A **bid** is a bidder's signed offer against it, keyed by `bidId`. Accepting a bid hands you either ready-to-sign calldata or a single API call, depending on the bid's settlement type. Funds never leave the user's wallet until the swap executes.
